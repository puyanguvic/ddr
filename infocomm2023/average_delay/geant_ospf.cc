...
std::string ExpName = "geant-topo-ospf";

NS_LOG_COMPONENT_DEFINE ("geantTopology");

int main (int argc, char *argv[])
{

  std::string format ("Inet");
  std::string input ("contrib/dgr/infocomm2023/topo/Inet_geant_topo.txt");
  ...
  Address sinkAddress (InetSocketAddress (ipic[28].GetAddress (1), sinkPort));
  DGRSinkHelper dsrSinkHelper ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort));
  ApplicationContainer sinkApps = dsrSinkHelper.Install (nodes.Get (14));
  sinkApps.Start (Seconds (0.));
  sinkApps.Stop (Seconds (20.));

  // Create a sender application
  Ptr<Socket> ns3udpSocket = Socket::CreateSocket (nodes.Get (16), UdpSocketFactory::GetTypeId ());
  uint32_t budget = 50;
  Ptr<DGRUdpApplication> app = CreateObject<DGRUdpApplication> ();
  app->Setup (ns3udpSocket, sinkAddress, 150, 1, DataRate ("1Mbps"), budget, true);
  nodes.Get (16)->AddApplication (app);
  ...
  // -- Net anim
  // ---------------------------------------------
  AnimationInterface anim ("geant_topo_ospf.xml");
