...
std::string ExpName = "4-point-topo";

NS_LOG_COMPONENT_DEFINE ("4PointTopology");

int main (int argc, char *argv[])
{

  std::string format ("Inet");
  std::string input ("contrib/dgr/infocomm2023/topo/4point_topo.txt");
  ...
  Address sinkAddress (InetSocketAddress (ipic[2].GetAddress (1), sinkPort));
  ...
  app->Setup (ns3udpSocket, sinkAddress, 200, 150, DataRate ("6Mbps"), budget, true);
  ...
  // -- Net anim
  // ---------------------------------------------
  AnimationInterface anim ("4point_topo_dgr.xml");
